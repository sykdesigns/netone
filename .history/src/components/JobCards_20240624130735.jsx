...
const JobCards = ({ postDate, position, industry, projectSummary, billingRateW2, billingRateIC, location, onViewDetails, onApplyNow }) => {
  ...
  return (
    <div className='relative flex flex-col lg:w-80 lg:h-70 bg-n1-blue-light p-4 rounded-lg shadow-md'>
      <div className='flex mb-2 text-2xl items-start'>
        ...
        <div className='ml-auto cursor-pointer hover:text-n1-orange' onClick={handleBookmarkClick}>
          {isBookmarked ? <BiSolidBookmark className='text-xl text-n1-blue' /> : <BiBookmark className='text-xl text-n1-blue' />}
        </div>
      </div>
      ...
      <div className='flex mt-4 ml-auto'>
        <button className='bg-n1-white border-2 border-n1-blue-mid text-n1-blue text-xs px-4 py-2 rounded-full mr-2 hover:bg-n1-orange hover:text-n1-white hover:border-n1-white' onClick={onViewDetails}>View Details</button>
        <button className='bg-n1-blue text-n1-white text-xs rounded-full px-4 py-2 hover:bg-n1-orange' onClick={onApplyNow}>Apply Now</button>
      </div>
      {showPopover && (
        <div className="absolute top-0 right-0 mt-2 mr-2 bg-white p-2 rounded shadow text-xs text-n1-blue">
          {isBookmarked ? "Job Bookmarked!" : "Bookmark Removed"}
        </div>
      )}
    </div>
  );
