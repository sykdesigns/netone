import React from 'react';
import { CiBookmark } from "react-icons/ci";
import { MdWork } from "react-icons/md";
import { SiChianetwork } from "react-icons/si";

const JobCards = ({ logo, postDate, position, industry, projectSummary, billingRateW2, billingRateIC }) => {
  return (
    <div className='flex flex-col w-80 h-72 bg-n1-white p-4 rounded-lg shadow-lg'>
      
        <div className='flex mb-2 text-2xl items-start'>

        <div className='bg-n1-blue-light w-12 h-12 rounded-full flex items-center justify-center text-n1-blue'>    
        <SiChianetwork />
        </div>
        <div className='flex flex-col ml-6'>
        <p className='text-xs text-n1-gray-dark'>3 days ago</p>
        <h2 className='text-lg text-n1-blue font-demiBold'>Technical Recruiter</h2>
        </div>
        <div className='ml-auto'>
        <CiBookmark className='text-xl text-n1-blue' />
        </div>
        </div>

       
        
        
      
     


      <div className='mb-2'>
        <p className='text-sm text-gray-300'>{industry}</p>
        <p className='text-sm'>{projectSummary}</p>
      </div>

      <div className='flex flex-row mb-2'>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Dictum non consectetur a erat nam at lectus urna. Sit amet dictum sit amet justo donec enim diam. Ullamcorper dignissim cras tincidunt lobortis feugiat vivamus at augue. Platea dictumst quisque sagittis purus sit amet volutpat consequat mauris. Scelerisque felis imperdiet proin fermentum leo vel orci. Ligula ullamcorper malesuada proin libero nunc consequat interdum varius. Euismod lacinia at quis risus sed vulputate odio ut. Consequat semper viverra nam libero justo laoreet sit amet cursus. Adipiscing tristique risus nec feugiat in fermentum posuere. Magnis dis parturient montes nascetur ridiculus. Auctor urna nunc id cursus metus aliquam. Arcu felis bibendum ut tristique. Interdum consectetur libero id faucibus nisl tincidunt.</p>
        <p className='text-sm text-n1-blue font-demiBold mr-4'>W2: {billingRateW2}</p>
        <p className='text-sm text-n1-blue font-demiBold'>IC: {billingRateIC}</p>
      </div>

      <div className='flex justify-center'>
        <button className='bg-n1-white border-2 border-n1-blue-mid text-n1-blue text-xs px-4 py-2 rounded-full mr-2'>View Details</button>
        <button className='bg-n1-blue text-n1-white text-xs rounded-full px-4 py-2'>Apply Now</button>
      </div>
    
    
    </div>
  );
}

export default JobCards;
